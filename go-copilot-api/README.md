...
| `HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY` | HTTP proxy URL for outgoing requests | *(none)*               |

**HTTP Proxy Support:**
- The server supports routing all outgoing HTTP requests through a proxy.
- Set one of the following environment variables: `HTTPS_PROXY`, `https_proxy`, `HTTP_PROXY`, `http_proxy`, `ALL_PROXY`, or `all_proxy`.
- Example: `HTTPS_PROXY=http://proxy.example.com:8080` or `HTTPS_PROXY=socks5://localhost:1080`
- When a proxy is configured, it will be used for all requests to GitHub Copilot APIs.
